...
| TD-TO-001 | Sync provider timeout cannot kill threads | Abandoned work may continue | Bounded pool; eval cases use child-process hard timeout | Phase 3 | Prefer async providers |
| TD-STREAM-001 | Streaming fallback limitations | Partial streams | Cancel checks between chunks | Phase 3 | Full async streaming |
| TD-EVAL-001 | Real-model evaluation incomplete | Quality unknown | Phase 3A mock/deterministic provider eval + hard timeouts | Phase 3B | Real local model eval reports |
| TD-DATA-001 | Tracked ssn/data mutated by tests (pre-3A) | Dirty worktrees / CI noise | Per-test SSN_RUNTIME_DATA_DIR isolation | Phase 3A | Governed suites leave tracked data clean |
| TD-SEC-001 | Local provider not production-hardened | Misconfig / redirect risk | Redirect reject; request sanitizer; conservative caps | Phase 3B+ | External security review |
