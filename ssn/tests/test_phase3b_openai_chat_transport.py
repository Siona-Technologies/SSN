...
class TestOpenAIDialectBehaviour(unittest.TestCase):
    ...
    def test_tool_calls_ignored_not_executed(self):
        ...
        try:
            ...
            self.assertEqual(resp.meta.get("provider_tool_calls_observed_count"), 1)
            self.assertTrue(resp.meta.get("provider_tool_calls_observed"))
            self.assertTrue(resp.meta.get("provider_tool_calls_ignored"))
            caps = p.capabilities()
